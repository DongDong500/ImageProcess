...
if __name__ == "__main__":
    ...
    src = 'c:/Users/singku/google drive ku 175T/code/Ansan/data/CTS/test'
    
    file_list = [img for img in os.listdir(src)]
    
    mask = os.path.join(src, file_list[0])
    img = os.path.join(src, file_list[1])
    
    mask_img = cv2.imread(mask)
    img_img = cv2.imread(img)
    tmp = img_img - mask_img
    cv2.imshow('mask', tmp)
    cv2.waitKey(0)
...
# 내가 만든 마우스 함수를 실행시켜줘
    ...
    if k == 27:
        ...
        if not cv2.imwrite(r'./mask.png', img):
